			...
			for ($i = 0; $i < 7; $i++)
			{
				...
				if ($day > 0 AND $day <= $total_days)
				{
					...
					{	
						...
						if ($i == 0) {$temp = str_replace('{first_day}', " first_day", $temp);} else {$temp = str_replace('{first_day}', "", $temp);}
						if ($i == 6) {$temp = str_replace('{last_day}', " last_day", $temp);} else {$temp = str_replace('{last_day}', "", $temp);}
						if ($i == (0-$start_day < 0 ? 0-$start_day+7 : 0-$start_day) ||
						    $i == (6-$start_day)) { $temp = str_replace('{is_weekend}', " weekend", $temp);} else {$temp = str_replace('{is_weekend}', "", $temp);}
