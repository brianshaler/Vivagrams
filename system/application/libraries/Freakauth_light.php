...
class Freakauth_light
{
    ...
    */
            ...
            else
            {
                ...
                if (isset($_SERVER['HTTP_REFERER']))
                {
                    ...
                    else
                    {
                        ...
                        // loop (or whatever, I don't remember in fact!), we do this:
    ...
    function _sendEmail($email, $subject, $message)
    {
        ...
        $config['protocol'] = "sendmail"; // mail/sendmail/smtp        $config['protocol'] = 'smtp';
        $config['smtp_host'] = 'relay-hosting.secureserver.net';
        $config['smtp_user'] = 'shaler2k@shaler.name';
        $config['smtp_pass'] = 'mohawk123';
        $config['smtp_port'] = '25';
...
//        $tobj->email->from($this->CI->config->item('FAL_user_support'), $this->CI->config->item('FAL_website_name').' '.$this->CI->config->item('FAL_email_from'));
        $tobj->email->from("brian@shaler.name", "Brian Shaler");
        $tobj->email->to($email);
        $tobj->email->subject($subject);
        $tobj->email->message($message);
        $tobj->email->set_alt_message($message);
        $tobj->email->send();
    }
